...
description: Interactive walkthrough of the delta_lake_and_polars_basic repo — guides a new owner from "just opened the repo" to "understands every component and the WHY behind it". Also acts as an on-call mentor that follows the user's specific context (current file / current question / a goal they're working toward), not just a linear curriculum. Use when onboarding into this project, when asked to teach this codebase, when the user has a specific spot they want unpacked, when they're stuck on what to do next, or when they want help extending the project beyond the curriculum.
